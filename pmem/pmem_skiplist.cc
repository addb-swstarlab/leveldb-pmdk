...
namespace leveldb {
  ...
  void PmemSkiplist::InsertByOID(PMEMoid* node, PMEMoid *key_oid, PMEMoid *value_oid, 
                                      int key_len, int value_len, int index) {
    uint64_t actual_index = GetActualIndex(&free_list_, &allocated_map_, index);
    int result = map_insert_by_oid(mapc, map[actual_index], &current_node[actual_index],
                                    node, key_oid, value_oid,
  ...
  void PmemSkiplist::GetNextTOID(int index, char *key, 
    ...
    uint64_t actual_index = GetActualIndex(&free_list_, &allocated_map_, index);
    map_get_next_TOID(mapc, map[actual_index], key, prev, curr);
  }
  ...
  PMEMoid* PmemIterator::node() const {
    return current_;
  }
